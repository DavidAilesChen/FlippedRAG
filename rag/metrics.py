...
def evaluate_models(results):
    ...
    for model in results.keys():
        preds = results[model]['preds']#logit
...
def set_recall(rank_truth_dict, rank_dict ,topk = 4):
    accumulate = []
    for query in rank_dict.keys():
        truth_list = [t[0] for t in sorted(rank_truth_dict[query].items(), key = lambda e:e[1])]
        pred_list = [t[0] for t in sorted(rank_dict[query].items(), key = lambda e:e[1], reverse=True)]
        inter_rate = len(set(truth_list[:topk]) & set(pred_list[:topk]))/topk
        print("TOP4:", pred_list[:topk], "TRUTH:", truth_list[:topk])
        accumulate.append(inter_rate)
    return sum(accumulate)/len(accumulate)
