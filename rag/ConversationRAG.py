...
class ConversationRAGChain(object):
    def __init__(self, llm, prompt_llm, retriever) -> None:
        self.llm = llm#huggingface
...
# or formulate a standalone question 
        self.prompt_qa = """
    ...
    # Use the following pieces of retrieved context to answer the question. Keep the answer concise:
    # Context: {context}.
    #    Answer the question in JSON. The output JSON has only one key 'answer', only output the JSON ['answer':].
