...
import torch
from transformers import BertModel, BertForSequenceClassification,  BertForNextSentencePrediction, BertForMaskedLM, XLMRobertaForMaskedLM, AutoModelForCausalLM, AutoModelForMaskedLM, XLMRobertaModel
from transformers.models.bert.modeling_bert import BertEmbeddings
from ir.bert_layers import BertEncoderPast
from FlagEmbedding import BGEM3FlagModel
from FlagEmbedding.BGE_M3 import BGEM3Model
...
class BertAutoRegressiveModel(BertModel):
    ...
    def forward(
        ...
        outputs = (sequence_output, pooled_output,) + encoder_outputs[1:]#sequence_output, pooled_output, past
...
class BertForLM(BertForMaskedLM):
    ...
    def forward(
        ...
        sequence_output = outputs[0]#[num_beams, 1, 768(bert embedding)]
        ...
        return outputs#[prediction_scores, encode_past]

class BGEForLM(BGEM3Model):
    def __init__(self, config, device):
        super().__init__(config)
        self.encoder = BGEM3FlagModel('BAAI/bge-m3', use_fp16=False, device=device)
        self.start_idx = 1
        # self.xml_roberta = self.encoder.model.model
        self.xml_roberta = XLMRobertaModel.from_pretrained("xlm-roberta-large")
        self.bge_inference = self.encoder.model
        # self.xml_roberta_mask_lm = XLMRobertaForMaskedLM.from_pretrained('BAAI/bge-m3')
        self.xml_roberta_mask_lm = AutoModelForMaskedLM.from_pretrained("xlm-roberta-large")
        self.lm_head = self.xml_roberta_mask_lm.lm_head
        # self.xml_roberta.init_weights()
        self.xml_roberta.config.is_decoder = True#To return past_value
    ...
    def forward(
        ...
        outputs_state = self.xml_roberta(
            input_ids,
    ...
    def parameters(self):
        return self.xml_roberta.parameters()
    
    def to(self, device):
        self.encoder.device = device
        self.xml_roberta = self.xml_roberta.to(device)
        self.lm_head = self.lm_head.to(device)

    def get_input_embeddings(self):
        return self.xml_roberta.get_input_embeddings()
