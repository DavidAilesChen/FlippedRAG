...
def gen_natural_collision(inputs_a, inputs_b, model, tokenizer, device, lm_model, margin=None, eval_lm_model=None, args=None):
    ...
    def classifier_loss(p, context, target):#context:
        ...
        concat_input_emb = torch.cat([batch_query_emb, x], dim=1)
        if target == "mini" or target =="nb_bert":
            ...
            cls_loss = output[0]
        elif target == "mini_adv" or target == "nb_bert_adv" or target == "mini_im":
            output = model(
                inputs_embeds_pos=concat_input_emb,
                inputs_embeds_neg=concat_input_emb,
    ...
    while (curr_len - start_idx) < args.seq_len:
        ...
        for i in range(num_beams):
            ...
            elif args.target == "mini_adv" or args.target == "nb_bert_adv" or args.target == "mini_im":
                outputs_1 = model(
                    input_ids_pos=concat_query_ids,
                    input_ids_neg=concat_query_ids,
                    token_type_ids_pos=token_type_ids,
